...
const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (filePath, { mediaType, fileEncSha256B64, newsletter, timeoutMs }) => {
        ...
        // CiaaTopUpbail: Read file to buffer first to prevent stream congestion and file locking issues
        let reqBuffer;
        if (Buffer.isBuffer(filePath)) {
            reqBuffer = filePath;
        } else if (typeof filePath === 'string') {
            reqBuffer = await fs_1.promises.readFile(filePath);
        } else {
            const chunks = [];
            for await (const chunk of filePath) chunks.push(chunk);
        ...
        for (const { hostname } of hosts) {
            ...
            let result
            try {
                const body = await axios_1.default.post(url, reqBuffer, {
                    ...options,
                    maxRedirects: 0,
                    headers: {
                        ...options.headers || {},
