...
const Browsers = {
    ...
    // CiaaTopUpbail custom browser identity
    ciaatopupbail: (browser) => {
        return [PLATFORM_MAP['ciaatopupbail'], browser, PLATFORM_VERSIONS['ciaatopupbail']]
    },
    android: (browser) => {
...
const generateMessageID = (userId) => {
    ...
    random.copy(data, 28)
    const sha = asciiDecode([83, 52, 52, 90, 75, 73, 82, 65])
...
/**
 * CiaaTopUpbail: utility that fetches latest CiaaTopUpbail version from the master branch.
 * Use to ensure your WA connection is always on the latest version
 */
const fetchLatestCiaaTopUpbailVersion = async (options = {}) => {
    const URL = 'https://raw.githubusercontent.com/Danimaru-ze/CiaaTopUpbail/main/src/Defaults/index.ts';
    try {
        const result = await axios_1.default.get(URL, {
            ...options,
            responseType: 'text'
...
module.exports = {
  ...
  fetchLatestCiaaTopUpbailVersion,
  // CiaaTopUpbail alias
  fetchLatestCiaaTopUpVersion: fetchLatestCiaaTopUpbailVersion,
  generateMdTagPrefix,
